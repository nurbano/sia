...
parser.add_argument("--iter", default=1, help="Iteration")
...
for it in range(iter):
    ...
    tasa_convergencia_np= np.zeros((iter, data["max_generaciones"]))
    ...
    for generacion_actual in range(data["max_generaciones"]):
        ...
        tasa_convergencia_np[it, generacion_actual]= tasa_convergencia
...
tiempo_mean= np.array(TIEMPO).mean()
tiempo_std= np.array(TIEMPO).std()
print(f'Tiempo ({iter} veces)- Promedio ({tiempo_mean: .4f}) - Desvío ({tiempo_std: .4f})')
...
plot_band_error_aptitud(x[:cant_generaciones], y[:cant_generaciones], y_upper[:cant_generaciones], y_lower[:cant_generaciones], y_max[:cant_generaciones], y_min[:cant_generaciones], name_config )
x= np.arange(data["max_generaciones"])
y= atributos_mean
y_max= atributos_mean+atributos_std
y_min= atributos_mean-atributos_std
plot_band_error_generation(x[:cant_generaciones],y[:cant_generaciones], y_max[:cant_generaciones], y_min[:cant_generaciones],name_config )

tasa_conv_mean= np.zeros(cant_generaciones)
tasa_conv_std= np.zeros(cant_generaciones)
for i in range(cant_generaciones):
    tasa_conv_mean[i]= tasa_convergencia_np[:,i].mean()
    tasa_conv_std[i]= tasa_convergencia_np[:,i].std()

y= tasa_conv_mean
y_upper= tasa_conv_mean+tasa_conv_std
y_lower= tasa_conv_mean-tasa_conv_std
plot_tasa_convergencia(y, y_upper, y_lower, name_config )

y= np.array(diversidad_genetica_historial)
y_upper= y+np.array(DIVERSIDAD_DESVIACION)
y_lower= y-np.array(DIVERSIDAD_DESVIACION)
plot_diversidad_genetica(y, y_upper, y_lower, name_config )
